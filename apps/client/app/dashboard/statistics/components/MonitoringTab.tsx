    ...
    return (
      <div className="flex h-96 items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-500">전체 서비스 통계를 취합하는 중입니다...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 h-full flex flex-col">
        ...
        <div className="flex-none space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <span className="text-gray-500 font-medium">전체 토큰 사용량</span>
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div className="text-3xl font-bold text-gray-900">${stats.totalCost.toFixed(4)}</div>
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="mb-6 pb-6 border-b border-gray-100">
                    ...
                    <div className="space-y-3">
                        {stats.topExpensiveModels && stats.topExpensiveModels.length > 0 ? (
                                ...
                                <div key={idx} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg border border-gray-200">
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold text-sm text-gray-900">{model.model_name}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {stats.top3Expensive?.map((service) => (
                        <div key={service.id} className="p-4 rounded-lg bg-gray-50 border border-gray-200 flex flex-col gap-3 relative overflow-hidden">
                            {/* No Rank Badge */}
                            
                            <div 
                                className="font-bold text-gray-900 truncate cursor-pointer hover:text-blue-600 transition-colors"
                                onClick={() => router.push(`/workflows/${service.id}`)}
                            >
                                {service.name}
                            </div>
                            ...
                            <div className="space-y-1.5">
                                <div className="flex justify-between items-center text-sm">
                                    ...
                                    <span className="font-bold text-gray-900">${service.cost.toFixed(4)}</span>
                                </div>
                                <div className="flex justify-between items-center text-sm">
                                    <span className="text-gray-500">사용 토큰</span>
                                    <span className="font-medium text-gray-700">{service.tokens.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-center text-sm">
                                    <span className="text-gray-500">실행 횟수</span>
                                    <span className="font-medium text-gray-700">{service.runs.toLocaleString()}회</span>
                                </div>
                            </div>
                        </div>
                    ))}
                        ...
                        {service.stats ? (
                            <div className="space-y-2">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">{service.stats.summary.totalRuns.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500">성공률</span>
                                    <span className={`font-medium ${
                                        service.stats.summary.successRate >= 90 ? 'text-green-600' : 
                                        service.stats.summary.successRate >= 70 ? 'text-orange-600' : 'text-red-600'
                                    }`}>
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">
                                        {Math.round(service.stats.summary.avgTokenPerRun).toLocaleString()}
                                    </span>
                                </div>
                                <div className="flex justify-between text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {stats.recentFailures.map((fail) => {
                                ...
                                <tr 
                                    ...
                                    onClick={() => router.push(`/workflows/${fail.workflow_id}`)}
                                >
