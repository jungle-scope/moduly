...
export default function SettingsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!provRes.ok) {
        throw new Error(provData.detail || 'Failed to fetch providers');
      }
      ...
      if (!credRes.ok) {
        throw new Error(credData.detail || 'Failed to fetch credentials');
      }
          ...
          <div className="mx-auto max-w-5xl space-y-8">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                시스템이 지원하는 Provider에 본인의 API Key를 등록하여
                사용하세요.
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="grid gap-6">
                {providers.map((provider) => {
                  const myCreds = credentials.filter(
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <div className="text-xs text-gray-400 flex gap-2">
                              ...
                              <span>•</span>
                              <span>
                                Models: {provider.models.length}개 지원 (
                                {provider.models
                                  .slice(0, 3)
                                  .map((m) => m.name)
                                  .join(', ')}
                            ...
                            {myCreds.map((cred) => (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div
                                    className={`p-1.5 rounded-full ${cred.is_valid ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}
                                  >
                                    {cred.is_valid ? (
