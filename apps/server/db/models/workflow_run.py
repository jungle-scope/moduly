...
from sqlalchemy import DateTime, String, Text, ForeignKey, Float, Integer, Numeric
...
class WorkflowRun(Base):
    ...
    total_cost: Mapped[Optional[float]] = mapped_column(Numeric(10, 6), nullable=True, default=0.0)

    # === 배포 정보 (선택) ===
    # 배포된 버전으로 실행된 경우 연결
    deployment_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("workflow_deployments.id", ondelete="SET NULL"), nullable=True
    )
    workflow_version: Mapped[Optional[int]] = mapped_column(Integer, nullable=True) # 배포 버전 스냅샷
