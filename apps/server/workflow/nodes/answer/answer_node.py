    ...
    def _run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for output_config in self.data.outputs:
            ...
            if not selector or len(selector) < 2:
                print(f"[{self.data.title}] 경고: 잘못된 selector 형식 - {variable_name}")
            ...
            if source_data is None:
                print(f"[{self.data.title}] 경고: 데이터를 찾을 수 없음 - Node: {target_node_id}")
                result[variable_name] = None
                continue
            ...
            if isinstance(source_data, dict):
                ...
                print(f"[{self.data.title}] 참고: 소스 데이터가 Dict가 아님 - {target_node_id}")
