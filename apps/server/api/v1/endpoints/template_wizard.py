...
4. 변수 형식은 반드시 {{ variable_name }} (중괄호 2개 + 공백) 형태를 유지하세요"""
...
def improve_template(
    ...
    try:
        ...
        if request.registered_variables:
            variables_str = ", ".join([f"{{{{ {v} }}}}" for v in request.registered_variables])
        else:
        ...
        messages = [
            ...
            {"role": "user", "content": f"다음 템플릿을 개선해주세요:\n\n{request.original_template}"}
        ]
        ...
        # 7. 응답 파싱
        improved_template = response.get("choices", [{}])[0].get("message", {}).get("content", "")
        ...
        raise HTTPException(status_code=500, detail=f"템플릿 개선 중 오류 발생: {str(e)}")
