import pytest
from workflow.nodes.answer.answer_node import AnswerNode
from workflow.nodes.answer.entities import AnswerNodeData, AnswerNodeOutput
...
def test_answer_node_execution():
    ...
    node_data = AnswerNodeData(
        title="결과 노드",
        outputs=[
            AnswerNodeOutput(variable="user_input", value_selector=["start_node", "query"]),
            AnswerNodeOutput(variable="ai_response", value_selector=["llm_node", "text"]),
        ]
    )
...
def test_answer_node_missing_input():
    ...
    node_data = AnswerNodeData(
        title="결과 노드",
        outputs=[
            AnswerNodeOutput(variable="missing_var", value_selector=["missing_node", "key"]),
        ]
    )
    node = AnswerNode(id="answer_node", data=node_data)
